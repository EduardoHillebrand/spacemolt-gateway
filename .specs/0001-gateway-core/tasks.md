...
- [x] **001_init-repo** — `git init`, `.gitignore` de Python, `README`, `CLAUDE.md`,
      pasta `.specs/` e `.claude/skills/` commitados. Primeiro commit na master.
- [x] **002_project-skeleton** — criar `app/`, `tests/`, `pyproject.toml`,
      ambiente virtual, `pytest` rodando (mesmo sem teste de verdade ainda).
- [x] **003_game-client** — `app/game_client.py` com `call(tool, **args)` e a
      sessão isolada. Teste com cliente MCP de mentira.
- [x] **004_mcp-server** — `app/server.py` sobe um MCP vazio. Teste de fumaça:
      o servidor lista zero ou as ferramentas registradas.
- [x] **005_raw-proxy** — `app/registry.py` registra 2–3 ferramentas cruas
